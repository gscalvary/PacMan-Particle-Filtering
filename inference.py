...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        posIndex = 0
        posIndexMax = len(self.legalPositions) - 1
        
        for index in range(self.numParticles):
            self.particles.append(self.legalPositions[posIndex])
            if posIndex == posIndexMax:
                posIndex = 0
            else:
                posIndex += 1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # build a cartesian product of legal positions
        self.posProduct = itertools.product(self.legalPositions, repeat=self.numGhosts)
        
        # build a list of the position combinations
        jointLegalPositions = []
        
        for element in self.posProduct:
            jointLegalPositions.append(element)

        # construct the particle list
        jointPosIndex = 0
        random.shuffle(jointLegalPositions)
        jointPosIndexMax = len(jointLegalPositions) - 1
        self.particles = []
        
        for index in range(self.numParticles):
            self.particles.append(jointLegalPositions[jointPosIndex])
            if jointPosIndex == jointPosIndexMax:
                jointPosIndex = 0
                random.shuffle(jointLegalPositions)
            else:
                jointPosIndex += 1
